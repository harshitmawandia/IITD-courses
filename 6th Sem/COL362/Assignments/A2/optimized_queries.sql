...
validcommentliking(personid ,creatorid, mid) as(
    ...
    FROM comment
        Join person_likes_Comment pkp On comment.id = pkp.commentid and comment.length > :commentlength
...
--P3--
create index id1 on comment_hastag_tag(creationdate);
create index id2 on post_hastag_tag(creationdate);
...
--P5--
CREATE INDEX id1 ON post(containerforumid);
CREATE INDEX id2 ON tagclass(id);
